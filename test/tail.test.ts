...
test(`w/ str = undefined`, () => {
  const being = tail(arr);
  expect(arr.concat(undefined as any).map(being)).toMatchInlineSnapshot(`
    Array [
      "foobarbaz        ",
      "foobar           ",
      "foo              ",
...
test(`w/ str = undefined & empty arr`, () => {
  const being = tail([]);
  expect(arr.concat(undefined as any).map(being)).toMatchInlineSnapshot(`
    Array [
      "foobarbaz        ",
...
test(`w/ str = undefined & negative spaces`, () => {
  const being = tail(arr, -10);
  expect(arr.concat(undefined as any).map(being)).toMatchInlineSnapshot(`
    Array [
      "foobarbaz ",
